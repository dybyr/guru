...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //완료 버튼을 누르면 데이터베이스에 선택한 알러지가 저장됨
        btndone = findViewById(R.id.btndone)
        btndone.setOnClickListener {
            saveSelectedItemsToDatabase() //클릭 이벤트가 발생했을 때, saveSelectedItemsToDatabase() 함수를 호출하고 이 함수는 선택한 알러지를 데이터베이스에 저장하는 역할을 함
        }

        //알러지를 다시 선택한 후에 수정 버튼을 누르면 수정된 알러지가 데이터베이스에 저장됨
        btnchange = findViewById(R.id.btnchange)
        btnchange.setOnClickListener {
            updateDatabase()  // 클릭 이벤트가 발생했을 때, updateDatabase()함수를 호출하고, 이 함수는 다시 선택한 알러지를 데이터베이스에 업데이트하여 저장하는 역할을 함
        }
    }
    ...
    private fun saveSelectedItemsToDatabase() {
        ...
        // 선택된 알러지 항목들이 담긴 리스트를 받아서 데이터베이스에 저장함
    ...
    private fun updateDatabase() {
        ...
        for (i in 0 until listView.count) {
            if (listView.isItemChecked(i)) { //현재 위치(i)의 알러지 항목이 선택되었는지 확인함
        ...
        try {
            // 기존 테이블에서 모든 데이터를 삭제함
            db.delete(DBHelper.TABLE_NAME, null, null)

            // 업데이트된 선택 항목들을 데이터베이스에 저장함
            for (item in selectedItems) {
                val values = ContentValues().apply { //ContentValues()를 사용하여 데이터베이스에 추가할 값을 설정함
                    put(DBHelper.COLUMN_NAME, item)
                }
                db.insert(DBHelper.TABLE_NAME, null, values) //db.insert를 사용하여 데이터베이스 테이블에 행을 추가함
            }
